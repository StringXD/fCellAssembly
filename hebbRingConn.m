...
[~,I] = sort(metadata{midx}(:,1));
metadata{midx} = metadata{midx}(I,:);
end

hebbMem = [125;234;248;291;10039;10067];
hebbRings = cell(0,2);
for midx=1:3
    currData = metadata{midx};
    TF = find(currData(:,1) == 46 & all(ismember(currData(:,2:end),hebbMem),2));
    if isempty(TF)
        continue;
    end
    for ridx = TF(:)'
        hebbRings{end+1,1} = currData(ridx,2:end);
    end
end

folder = regexp(sorted_fpath{46},'(\w|\\|-)*','match','once');
[folderType,file,spkFolder,metaFolder,~]=jointFolder(folder,cell(0),homedir);
currmodel='selec';
sustIds=[];nonselIds=[];
for ridx = 1:length(hebbRings)
    transIds = hebbRings{ridx,1}';
    ...
    % startSuId startTs endSuId endTs trialId msize su1 ... su5(if possible)
    for tidx=cfg.trials(:)'
        ts_id=[];
        for seqid=1:msize
        ...
        [~,s]=sort(ts_id(:,1));
        ts_id=ts_id(s,:);
        tw_tagged=relax_tag(ts_id,msize);
        if isempty(tw_tagged)
            continue;
        end
        contRingNum = size(tw_tagged,1);
        tempInfoMat = -1*ones(contRingNum,11);
        tempInfoMat(:,1:4) = tw_tagged;
        tempInfoMat(:,5) = tidx;
        tempInfoMat(:,6) = msize;
        tempInfoMat(:,7:6+msize) = ones(contRingNum,1)*transIds(:)';
        hebbRings{ridx,2} = [hebbRings{ridx,2};tempInfoMat];
    end
end
